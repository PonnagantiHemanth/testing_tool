...
# ------------------------------------------------------------------------------
"""
@package pyhid.hidparser.helper

@brief  HID parser helper class
...
class ValueRange:
    ...
    def scale_to(self, new_range, value):
        ...
        if not self.in_range(value):
            raise ArithmeticError("value %d is outside of accepted range [%d .. %d]" % (value, self.minimum, self.maximum))
        value = (value - self.minimum) / (self.maximum - self.minimum)
        # Force the result to be an integer value
        return new_range.minimum + round(value * (new_range.maximum - new_range.minimum))
