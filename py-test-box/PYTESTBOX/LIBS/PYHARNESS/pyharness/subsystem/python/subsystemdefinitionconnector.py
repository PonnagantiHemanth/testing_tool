...
class PythonSubSystemDefinitionImporter(AbstractSubSystemDefinitionImporter):
    ...
    def _getNormedSysPath(self):
        ...
        if (self._normedSysPath is None):
            normedSysPath = [abspath(p) for p in sys.path]

            # Filter out members of the PYTHONPATH that are nested
            discardablePaths = set()
            for path in normedSysPath:
                for innerPath in normedSysPath:
                    if (    innerPath.startswith(path)
                        and (len(innerPath) > len(path))):
                        discardablePaths.add(path)
                    # end if
                # end for
            # end for

            self._normedSysPath = [path for path in normedSysPath if path not in discardablePaths]
        # end if
    ...
    def _loadFromFeaturePy(self, featurePyPath):
        ...
        for sysPath in self._getNormedSysPath():
            ...
            if (upperFullPath.startswith(sysPath.upper())) and (fullPath[len(sysPath)] == sep):

                relativePath = fullPath[len(sysPath) + len(sep):]
                fqn = '.'.join([x for x in splitext(relativePath)[0].split(sep) if len(x)])
                break
            # end if
        # end for

        # Has a theoretical fqn been found ?
        if (fqn is not None):
    ...
    def _loadFromLegacySubSystem(self, legacySubSystem):
        ...
        doc = legacySubSystem.__doc__ if legacySubSystem.__doc__ is not None else ''
        ...
        for childName in [n for n in dir(legacySubSystem) if n.startswith('F_')]:
            ...
            choicesName = 'C_' + childName[2:]
            childChoices = getattr(legacySubSystem, choicesName, None)

            editableName = 'U_' + childName[2:]
            editable = getattr(legacySubSystem, editableName, False)

            aliasesName = 'A_' + childName[2:]
            aliases = getattr(legacySubSystem, aliasesName, tuple())


            features.append(SubSystemDefinition.FeatureDefinition(childName[2:],
                                                                  default  = StringUtils.pythonToString(child, childType),
                                                                  doc      = childDoc,
                                                                  type     = childType,
    ...
    def _loadFromPackagePath(self, inputPath):
        ...
        for subDir in (sd for sd in listdir(inputPath) if ((sd not in self.IGNORED_SUBDIRECTORIES)
        ...
        processed = set()
        for subFile in (sf for sf in listdir(inputPath) if ((self._acceptableModule(splitext(basename(sf))[0]))
                                                        and (isfile(join(inputPath, sf))))):
            moduleName = splitext(basename(subFile))[0]
...
class PythonSubSystemDefinitionExporter(AbstractSubSystemDefinitionExporter):
    ...
    def _saveToFeaturePy(self, rootPath, subSystemDefinition, baseName = None):                                         # pylint:disable=R0912
        '''
        ...
        @param baseName            [in] (str) basename of the serialized file.
        '''
            ...
            def serializeSubsystemDefinitionToLines(_subSystemDefinition):
                ...
                for featureDefinition in _subSystemDefinition.features:
                    ...
                    if (featureDefinition.choices is not None):
                        lines.append('    self.C_%s = %r' % (featureDefinition.name, featureDefinition.choices)) # Choices
                    # end if

                    if (featureDefinition.editable):
                        lines.append('    self.U_%s = %r' % (featureDefinition.name, featureDefinition.editable)) # Choices
                    # end if

                    if (featureDefinition.aliases):
                        lines.append('    self.A_%s = %r' % (featureDefinition.name, featureDefinition.aliases)) # Aliases
                    # end if
